...
- ✅ **Preview tooltips** (optional): Hover/click a block in the sidebar for a small styled popover (`description` from `@block`). Toggle in **Admin → Design System → LaraGrape editor**, or set `LARAGRAPE_BLOCK_PREVIEW_TOOLTIPS=false` in `.env`.
